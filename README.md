...
위험 단계는 두 모델의 출력을 함께 사용해 산정할 수 있다. 다만 단순히 “확률이 높으면 경계, 더 높으면 고위험”처럼 나누면 공모전 관점의 의미가 약하다. 본 프로젝트의 차별점은 **위험 확률, 예상 세포수, SHAP 원인 피처를 함께 사용해 원인별 대응 시나리오를 출력하는 것**이다. 이처럼 분류 모델은 “위험 여부”, 회귀 모델은 “예상 규모”, SHAP은 “왜 위험한지”를 담당한다.
...
시나리오 출력은 위험 단계만으로 고정하지 않고, **주요 원인 피처 묶음**에 따라 달라져야 한다.

| 원인 시나리오 | 주요 SHAP 피처 예시 | 해석 | 관리자 대응 시나리오 |
| --- | --- | --- | --- |
| 현재 조류량 급증형 | `log_target`, `cyano_cells_log10p1_robust`, `alert_encoded`, `Microcystis_log10p1_robust` | 이미 현재 조류 세포수와 경보 단계가 높아 다음 조사 시점 위험으로 이어질 가능성이 큼 | 채수 빈도 확대, 조류 종별 정밀 모니터링, 경보 단계 상향 검토 |
| 수문 정체형 | `outflow_7d_sum_robust`, `level_change_7d_robust`, `residence_proxy_robust`, `nutrient_stagnation_index_robust` | 물이 충분히 순환되지 않거나 체류 조건이 강화되어 조류가 증식하기 좋은 환경 | 방류·수위 운영 상태 점검, 정체 구간 집중 감시, 취수 위치·운영 조건 검토 |
| 강우·유입 부하형 | `rain_7d_sum_x_robust`, `inflow_7d_sum_robust`, `rainfall_robust`, `daily_rain_robust` | 강우 이후 유입 증가로 영양염류 유입 가능성이 커지고 후행 조류 증가 위험 존재 | 강우 후 3~7일 집중 모니터링, 유입부 수질 확인, 상류 오염원 유입 여부 점검 |
| 고온·계절 성장형 | `water_temp_minmax`, `air_temp_7d_mean_minmax`, `sin_season_minmax`, `acc_temp_7d_minmax` | 수온과 계절성이 조류 성장에 유리한 상태 | 고온기 선제 관측 강화, 표층 수온 변화 추적, 취약 지점 우선 점검 |
| 탁도·수질 악화형 | `turbidity_log10p1_robust`, `Chl_a_log10p1_robust`, `DO_minmax`, `pH_minmax` | 수질 상태 변화가 조류 증식 또는 수체 교란과 함께 나타남 | 현장 수질 재측정, Chl-a 및 탁도 변화 추적, 조류 원인 수질 항목 확인 |

최종 출력은 아래처럼 구성하는 것이 목표다.

```text
위험 단계: 경계
예측 근거:
- Logistic Regression 경보 확률이 높음
- HuberRegressor 예상 세포수가 경보 기준에 근접
- SHAP 상위 원인: rain_7d_sum_x_robust, inflow_7d_sum_robust, log_target

시나리오 해석:
- 강우·유입 부하형 위험
- 최근 강우와 유입 증가가 후행 조류 증가 위험을 높이는 상황
